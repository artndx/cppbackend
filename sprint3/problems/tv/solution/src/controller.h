...
private:
    ...
    [[nodiscard]] bool SelectChannel(std::istream& input, std::ostream& output) const {
        ...
        try{
            std::string data; 
            if(input >> data){
                /* Ошибка парсинга аргумента */
                auto selected_channel = ParseArgument(data);
                if(!selected_channel.has_value()){
                    output << "Invalid channel"sv << std::endl;
                    return false;
                }   
                tv_.SelectChannel(selected_channel.value());
                return true;
            } else {
                output << "Error: the SelectChannel command require one argument <channel-number>"sv << std::endl;
                return false;
            }
    ...
    std::optional<int> ParseArgument(std::string_view data) const{
        std::string result;
        for(char c : data){
            if('0' <= c && c <= '9'){
                result.push_back(c);
